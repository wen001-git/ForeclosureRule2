...
| 2026-05-29 | AI Agent (Codex) | v24 | 统一整理 BPS UI 截图：为所有手工补充截图添加 Figure 13-x 官方标题，改用相对路径，移除绝对 Windows 路径；同步英文版同位置截图标题 | docs/en/13 |
| 2026-05-29 | AI Agent (Codex) | v25 | 将 doc 13 引用的 9 张截图文件重命名为有业务含义的英文 slug 文件名，并同步更新中英文引用路径 | docs/en/13 |
...
- BPS Bankruptcy 面板中 `lien_status`、`claim_status` 的 Newrez 原始字段映射待进一步确认
...
| BPS 表 | Newrez 源表 | 提取层筛选条件 | 同步层筛选条件 | 去重逻辑 | 含完结贷款？ |
|---|---|---|---|---|---|
| `sync_loan_foreclosure` | `portnewrezfc` | 无（全量进 Redshift） | `fcreferraldate IS NOT NULL` + JOIN portfunding | 无 | ✅ 含（activefcflag=0） |
...
> **关键差异**：`sync_fcl_stage_info` 是唯一**排除完结贷款**的表——聚合概览页仅反映当前进行中的 FCL，与其他 4 张表（含完结历史）的人口不同，不应直接比较数量。  
...
| `activefcflag` | int | FCL 是否仍活跃（1=进行中 / 0=已完结） | 100% |
...
| Milestone > Judgement Hearing Set Date | 判决听证会预定日。仅 Judicial 州适用，Non-Judicial 州为空 | `timeline_judgement_hearing_set_date` | `fcjudgmenthearingscheduled` | 直接取值 | 11.9% |
| Milestone > Judgement Date | 法院判决发布日。Judicial 州专用 | `timeline_judgement_date` | `fcjudgmententered` | 直接取值 | 7.9% |
| Milestone > Projected Sale Date | 最新预计/排定拍卖日（动态更新） | `timeline_sale_date_projected_date` | `fcscheduledsaledate` | 直接取值 | 18.2% |
| Milestone > Sale Date Set | 已排定拍卖日。与 Projected 来自同一 Newrez 字段，BPS 按业务状态判断区分 ⚠️ | `timeline_sale_date_set_date` | `fcscheduledsaledate` | 直接取值 | 18.2% |
...
| Milestone > Third Party Sold Date | 第三方买家成交日（活跃 FCL 中 266 笔 `fcresults='3rd Party'`）。需联合 `fcresults` 字段判断是否为第三方购买 ⚠️ | `timeline_third_party_sold_date_date` | `fcsalehelddate` | 当 `fcresults='3rd Party'` 时取 `fcsalehelddate` | 2.0% |
| Milestone > Third Party Proceeds Received Date | 第三方拍卖款到账日（活跃 FCL 均未到达款项到账阶段） | `timeline_third_party_proceeds_received_date` | `fcl3rdpartyproceedsreceiveddate` | 直接取值 | 0.0% |
...
> - `timeline_sale_date_projected_date` 与 `timeline_sale_date_set_date` 来自同一字段 `fcscheduledsaledate`，区分逻辑由 BPS 内部状态判断。
...
| Actual Days > Judgement Hearing Set | 判决听证至判决发布的天数（Judicial 州专用） | `actual_judgement_hearing_set_days` | `fcjudgmententered` − `fcjudgmenthearingscheduled` |
...
| Summary > SMS Days in FCL | Servicer（Newrez/SMS）口径的 FCL 在途天数（消除数据截止日延迟） | `summary_sms_days_in_fcl` | `smsdaysinfc`, `dataasof` | **实时重算**：`smsdaysinfc + DATEDIFF(当天纽约时间, dataasof)` |
| Summary > Days in FCL | 投资者口径的 FCL 在途天数（消除数据截止日延迟，反映今日实际天数） | `summary_days_in_fcl` | `daysinfc`, `dataasof` | **实时重算**：`daysinfc + DATEDIFF(当天纽约时间, dataasof)` |
...
| Summary > Completed Foreclosure | 标记 FCL 是否已完结（布尔值） | `summary_completed_foreclosure` | `activefcflag` | `activefcflag = 0` → 1（已完结）；`activefcflag = 1` → 0（进行中） |
...
> - **数值解码**：`lmdeal`/`lmprogram`/`lmstatus`/`lmdecision`/`denialreason`/`borrowerintention` 等 6 个整型编码在 Redshift 层通过 `LEFT JOIN newrez.portnewrezdatadic` 解码为业务文本后写入 BPS（与 Hold 面板直接存储文本描述不同）。
...
| Status | `lmc_status` | varchar(256) | `lmstatus`（int） | portnewrezlm | ETL 解码为文本（如 `112` → `"Awaiting MI Approval"`） |
...
| Final Disposition | `final_disposition` | varchar(256) | `lmdecision`（int） | portnewrezlm | ETL 解码为文本（如 `6` → `"LMS Opened in Error"`，`99` → `"Pending"`） |
...
| UI 列 | 业务角色 | 补充说明 |
|---|---|---|
| **Deal** | LM 大类（策略方向） | 标识本轮止赎救济的整体路径：Evaluation（初始评估）→ 保房方案（Modification / Forbearance / Repayment Plan）→ 退出方案（Short Sale / DIL）；详见下方「LM 字段取值与业务含义 — Deal」 |
...
| `deal` 值 | 业务含义 | 借款人结果 | 典型顺序 |
|---|---|---|---|
| `Evaluation` | 初始评估：汇总借款人财务信息，确认适用 LM 类型，尚未承诺具体方案 | 不确定 | 第 1 步 |
| `Modification` | 贷款修改：永久变更利率 / 期限 / 本金，借款人继续持有房产 | 保留房产 | 第 2 步 |
| `Forbearance` | 临时宽限：暂缓还款 3~12 个月（监管要求 COVID 期间提供） | 暂时宽限 | 第 2 步（临时）|
| `Repayment Plan` | 还款计划：分期偿还欠款，不修改贷款条款 | 保留房产 | 第 2 步（轻度）|
| `Short Sale` | 短售：以低于贷款余额的价格出售房产，贷款方书面豁免差额 | 出售房产 | 第 3 步 |
| `DIL` | 以房抵债（Deed-in-Lieu）：借款人主动将产权转让给贷款方，协议终止贷款 | 放弃房产 | 第 4 步（最终）|

#### Program（具体方案）

`program` 字段是 `deal` 大类下的具体执行方案，来源于 Newrez `lmprogram`（int）经 ETL 解码：

| `program` 值 | 所属 Deal | 业务含义 |
|---|---|---|
| `Evaluation` | Evaluation | 通用评估流程（无特定方案） |
| `Bridger mod` | Modification | 我司（Bridger/Newrez）自有修改方案，非 GSE 标准方案 |
| `496.0` | Modification | 对应特定投资人 / 机构的修改方案代码（如 Fannie Mae Flex Mod 或类似标准方案） |
| `Short Sale` | Short Sale | 标准短售流程 |
| `Deed-in-Lieu` | DIL | 标准以房抵债流程 |

> 注：`program` 取值会随投资人指南变化而增减；`496.0` 等数字代码为 Newrez 内部方案 ID，具体对应关系见 Newrez 配置表。
...
| `lmc_status` 值 | 业务含义 |
|---|---|
| `Pending Financials` | 等待借款人提交财务材料（收入证明、银行账单等） |
| `Workout Denial` | 本轮 LM 已被拒绝（借款人不符合条件或拒绝配合） |
| `Document Follow-up` | 已收到部分材料，跟进补充缺失文件 |
| `DIL Title Ordered` | 以房抵债程序中，产权调查已下单（确认无其他留置权） |
| `Negotiate DIL liens` | 与次级留置权方（二抵贷款、HOA 欠款等）协商清偿条款 |
| `Awaiting MI Approval` | 等待抵押保险（Mortgage Insurance）公司审批本 LM 方案 |
...
> **验证 SQL**：见 附录 B — SQL-7（确认指定贷款有无破产记录）
...
| UI 列 | BPS 字段 | Newrez 原始字段 | 来源表 | Newrez字段 → BPS字段 |
|---|---|---|---|---|
| Status | `bankruptcy_status` | `bkstatus`（int） | portnewrezbk | 数值编码（Newrez 内部破产状态码） |
| Legal Status | `legal_status` | `bkstage`（int） | portnewrezbk | 数值编码（破产法律程序阶段码） |
| Status Date | `status_date` | `bkrcurrentstatusdate` | portnewrezbk | 直接取值（当前状态生效日期） |
| Chapter | `chapter` | `bkchapter`（int） | portnewrezbk | 直接取值（7 / 11 / 13） |
| Lien Status | `lien_status` | *(待确认)* | portnewrezbk | 可能来自 cramdown 或 adversarial 相关字段 |
| MFR Status | `mfr_status` | `mfrhearingresults`（int） | portnewrezbk | 数值编码（MFR 听证结果） |
| MFR Filed Date | `mfr_filed_date` | `mfrfileddate` | portnewrezbk | 直接取值 |
| Claim Status | `claim_status` | *(待确认)* | portnewrezbk | 可能来自 POC 相关字段 |
| Proof of Claim Date | `proof_of_claim_date` | `pocfileddate` | portnewrezbk | 直接取值 |
| Post Petition Due Date | `post_petition_due_date` | `bkpostpetitionduedate` | portnewrezbk | 直接取值（破产申请后贷款应付日） |

### 注意事项

- 本面板仅在贷款有破产申请记录时才有数据。UI 截图中 loanid=7727000088 显示 "No Rows To Show"（经 MCP 确认，该贷款在 `sync_loan_foreclosure_bankruptcy` 中无记录）。
- `bkstatus` 和 `bkstage` 为数值编码，BPS 可能直接存储数字（与 LM 面板的文本解码不同，BK 面板的编码解码状态待进一步核实）。
...
> **数据范围**：本页展示所有满足入库条件的活跃 FCL 贷款，当前约 **13,321 笔**（dataasof 2026-05-24）。每笔贷款按当前 BPS 阶段（`stage` 字段）归入对应分组，单行显示该阶段的天数统计。
>
> **源数据筛选条件（ETL 代码验证，与 Section 3.1 主 FCL 表独立）**：
> - **主筛选（进行中 FCL，`basic_data_pool_config.py` `GEN_FCL_STAGE` lines 1789-1791）**：`activefcflag = 1 AND fcremovaldate IS NULL AND (fcremovaldesc IS NULL OR fcremovaldesc = '')`——仅进行中且未撤销的活跃 FCL 贷款入库。⚠️ **完结贷款（`activefcflag=0`）不在本表中**，与主 FCL 表（`sync_loan_foreclosure` 包含完结贷款）行为不同。
> - **次筛选（预 FCL 贷款，lines 1809-1818）**：`activefcflag != 1 AND demandsentdate IS NOT NULL AND referral_start_date IS NULL AND delinquency_status IN ('D90', 'D120P')`——有 Demand Letter 但尚未转介的 90天+ 逾期贷款也包含（`DEMAND` 阶段）。
...
> - **与 Section 3.1 的差异**：主 FCL 表入库条件为 `fcreferraldate IS NOT NULL`（含历史完结贷款）；聚合概览页条件为 `activefcflag=1 AND fcremovaldate IS NULL`（仅当前活跃，更严格）。两者人口不同，数量不应直接对比。
...
`sync_fcl_stage_info` 中的日期字段来自 `portnewrezfc` 对应的 timeline 字段（同 Section 3.1）；天数字段由 BPS 系统在每日同步时计算。各 `*_in_lm_days` 和 `*_on_hold_days` 由系统结合 `portnewrezlm.activelmflag`、Hold 状态综合计算。
...
| 优先级 | `stage` 存储值（DB 全大写代码） | 判断条件（Newrez `portnewrezfc` 字段） | BPS 输出字段（`sync_fcl_stage_info`） |
|--------|-------------------------------|--------------------------------------|--------------------------------------|
...
| 3 | `PUBLICATION` | `publication_date IS NOT NULL`（Newrez 贷款实测恒为 0，见 Section 8 Q1） | `publication_start_date` · `publication_stage_days` · `publication_in_lm_days` · `publication_on_hold_days` |
| 4 | `SERVICE` | `servicecompletedate IS NOT NULL` AND 优先级 1～3 均未命中 | `service_start_date` · `service_stage_days` · `service_in_lm_days` · `service_on_hold_days` |
...
| 7 | `DEMAND` | `demandsentdate IS NOT NULL` AND `fcreferraldate IS NULL`（入库后不应出现，实测 0%） | `demand_start_date` · `demand_stage_days` · `demand_in_lm_days` · `demand_on_hold_days` |
...
> **关键要点**：
> - 判断使用的字段全部来自 `newrez.portnewrezfc`（同 Section 3.1 的 Newrez 来源表）
> - `fcscheduledsaledate` 是最高优先级——有拍卖日则直接归入 `SALE`，不论 judicial/non-judicial
...
| UI 列 | 序号 | BPS 字段 | Newrez 原始字段（`portnewrezfc`） | Newrez 填充率 / 说明 |
|---|---|---|---|---|
...
| Q7 | BK 面板的 `bankruptcy_status`、`legal_status` 来自 Newrez 数值编码（bkstatus、bkstage），当前 BPS 可能直接存储数字而非解码文本 | Bankruptcy 面板 Status/Legal Status 列 | 需进一步核实是否有解码逻辑（与 LM 面板的文本解码行为不同） |
...
| Q12 | `port.basic_data_loan_fcl.fcjudgment_end_date`（对应 Newrez `fcjudgmententered` / Capecodfive `foreclosure_judgement_date`）存储在 Redshift ETL 中间表，但**未被任何下游 ETL 查询引用**，不流入任何 BPS MySQL 字段。`bpms_dev.sync_loan_foreclosure.actual_judgement_hearing_set_days`（判决阶段历时天数）的 ETL 来源不明，推测在 BPS 应用层计算 | `timeline_judgement_date`（已连接 `fcjudgment_hearing_scheduled`，正常）；`actual_judgement_hearing_set_days`（来源不明）；所有 `var_*/target_*` 判决天数字段 | `fcjudgment_end_date` 是**设计预留字段**：命名遵循中间表阶段起/止惯例（`fcjudgment_hearing_scheduled`=阶段起，`fcjudgment_end_date`=阶段止）；跨 Servicer 归一化（Capecodfive `foreclosure_judgement_date` 也映射到此列）；当前不影响 BPS 展示；可作为未来 `actual_judgement_hearing_set_days` ETL 化（= `fcjudgment_end_date` − `fcjudgment_hearing_scheduled`）的数据来源。详见 doc 12 Section 15 |
...
| 值 | 含义 |
|----|------|
| `1` | FCL 进行中（Active Foreclosure） |
| `0` | FCL 已完结（Closed / Removed） |
| `NULL` | ⚠️ 历史遗留问题（见下） |

**问题**：Newrez 历史数据中存在一批贷款 `activefcflag IS NULL`，但这些贷款实际上仍处于 FCL 流程中，**并未完结**。Newrez 在这批历史记录中漏填了 `1`，字段留空。

#### 为什么 NULL 应视为"进行中"而非"已完结"？

在 BPS 的数据架构中，FCL **完结**有明确的多重证据：

- `fcremovaldate` 有值 → 明确的退出日期
- `fcremovaldesc` 有值 → 明确的退出原因（如 `'Borrower Reinstated'`、`'Loss Mitigation'`）

因此：**如果一笔贷款的 FCL 已经结束，会有明确的结束证据——不会只靠 `activefcflag = 0` 来标识**。

反之，若 `activefcflag IS NULL` 且 `fcremovaldate` 也为空，保守处理应视为"仍在 FCL 流程中"。

> 风险不对称性：**漏掉一笔在途的 FCL 贷款（误判为已完结）**，比**多追踪一笔已完结贷款（误判为进行中）**风险更大——前者会导致 BPS 展示缺口，后者最多多展示一行历史记录。
...
> 本附录收录 7 条可直接在 MySQL 中运行的 SQL，覆盖文档中所有通过 MCP 数据库查询所得的数值、分布及样本数据。
