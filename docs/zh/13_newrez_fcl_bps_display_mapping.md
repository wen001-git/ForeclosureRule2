...
> **📅 数据日期**：本文实测**均取自 prod**（`mysql_prod`/`redshift_prod`），无 dev。填充率/规模为单一最新快照口径，as-of **dataasof 截至 2026-05-24**（见 §2.1）；各表 as-of 汇总见 doc 02 头部「数据日期」统一声明。
...
| 2026-06-03 | AI Agent（Claude Opus 4.8） | v33 | §5 数值解码补充机制溯源（代码+DB 验证）：lmdeal/lmprogram/lmstatus/lmdecision/borrowerintention/denialreason 解码源 = **Redshift 字典表 `newrez.portnewrezdatadic`**（dev MySQL 无），解码 JOIN 在 [`basic_data_pool_config.py:835-840`](https://gitlab.bridgerinvestment.com/jli/prefectflow/-/blob/32a750a39c7eda989de991c47467979043e3d209/flow/basic_data/basic_data_config/basic_data_pool_config.py#L835-840)（BK 在 :367），非硬编码；LMDeal 字典 13 码、实测 8 码；并说明跨表 join 偶见 lmdeal=1→Evaluation 系快照时点伪差 | [basic_data_pool_config.py:835-840](https://gitlab.bridgerinvestment.com/jli/prefectflow/-/blob/32a750a39c7eda989de991c47467979043e3d209/flow/basic_data/basic_data_config/basic_data_pool_config.py#L835-840)；Redshift portnewrezdatadic；doc 数据字典 |
...
| 2026-06-02 | AI Agent（Claude Opus 4.8） | v30 | 【ETL 代码 + DB 实测核实后更正 Section 6 破产面板映射】对照 [`basic_data_pool_config.py:349-363`](https://gitlab.bridgerinvestment.com/jli/prefectflow/-/blob/32a750a39c7eda989de991c47467979043e3d209/flow/basic_data/basic_data_config/basic_data_pool_config.py#L349-363) 逐字段核实：① `status_date` 实为 `bkfileddate`（破产申请日），非 `bkrcurrentstatusdate`；② `lien_status`/`mfr_status`/`claim_status`/`mfr_filed_date` 在 Newrez 提取链路**硬编码 NULL**（非 `mfrhearingresults`/`mfrfileddate`），DB 实测 0/64（mfr_filed_date 3/64 疑非提取链路）；③ `bankruptcy_status` = `COALESCE(datadic[BKStatus]解码, bkstatus)`，实测含 Completed/Cancelled 等（早前 1→Active…5 映射不完整）。同步更正 doc 16 ⑤ 块A/块B | ETL 代码 [basic_data_pool_config.py:349-363](https://gitlab.bridgerinvestment.com/jli/prefectflow/-/blob/32a750a39c7eda989de991c47467979043e3d209/flow/basic_data/basic_data_config/basic_data_pool_config.py#L349-363)；doc 16 |
...
- ~~BPS Bankruptcy 面板中 `lien_status`、`claim_status` 的 Newrez 原始字段映射待进一步确认~~ **（2026-06-02 已核实）**：`lien_status` / `claim_status` / `mfr_status` / `mfr_filed_date` 在 ETL 提取层（[basic_data_pool_config.py:349-363](https://gitlab.bridgerinvestment.com/jli/prefectflow/-/blob/32a750a39c7eda989de991c47467979043e3d209/flow/basic_data/basic_data_config/basic_data_pool_config.py#L349-363)）**硬编码为 `NULL`**，未映射任何 Newrez 字段。详见 Section 6
...
| BPS 界面标签 | 业务含义 | BPS 展示字段 | Newrez 字段 | Newrez字段 → BPS展示字段 |
|---|---|---|---|---|
| Bid Approval > Approval Status | 竞拍出价的审批状态（BPS 内部列，FCL ETL 不写入） | `bid_approval_status` | *(无上游)* | **无 Newrez 来源**；列在 `basic_data_loan_foreclosure` DDL 存在（[`basic_data_pool_config.py:193`](https://gitlab.bridgerinvestment.com/jli/prefectflow/-/blob/32a750a39c7eda989de991c47467979043e3d209/flow/basic_data/basic_data_config/basic_data_pool_config.py#L193)）但**不在 INSERT 列清单内**，本应由 BPS 侧竞拍审批流程写入。**DB 实测 2026-06-07：0% 填充**（0/6152 RS · 0/89 BPS） |
...
| Bid Approval > Bid Amount | 竞拍出价金额（bid_approval_* 中唯一被 ETL 填充的列） | `bid_approval_bid_amount` | `fcbidamount` | ⚠️ **订正**：来源是 `fcbidamount`（[`basic_data_pool_config.py:272`](https://gitlab.bridgerinvestment.com/jli/prefectflow/-/blob/32a750a39c7eda989de991c47467979043e3d209/flow/basic_data/basic_data_config/basic_data_pool_config.py#L272)），**非 `fcapprbidprice`**；与 `summary_foreclosure_bid_amount` 同值。DB 实测填充 16 行（16/6152 · 16/89） |
...
| Summary > Current Step | FCL 当前进行步骤（Newrez 阶段描述 `fcstage`） | `summary_current_step` | `fcstage` | **= `fcstage` 直传**（代码 [`basic_data_pool_config.py:282`](https://gitlab.bridgerinvestment.com/jli/prefectflow/-/blob/32a750a39c7eda989de991c47467979043e3d209/flow/basic_data/basic_data_config/basic_data_pool_config.py#L282) `fc.fcstage as summary_current_step`）。⚠️ `currentmilestone` 虽在 `portnewrezfc` 存在（填充 62.7%），但 **PrefectFlow 全仓 0 引用、ETL 从不读取**；旧「currentmilestone 优先」结论经代码 + prod 实测（2026-06-10）证伪、已更正（见 §8 Q13） |
...
> - `summary_days_in_fcl` ← `daysinfc`：ETL 按**转介日 `fcreferraldate`** 重算 `datediff(referral_start_date, snapshot)+1`（仅 Active；[`basic_data_pool_config.py:1628`](https://gitlab.bridgerinvestment.com/jli/prefectflow/-/blob/32a750a39c7eda989de991c47467979043e3d209/flow/basic_data/basic_data_config/basic_data_pool_config.py#L1628)）——**投资人/全程**口径。
...
> - BPS 两者都做实时修正 `+ DATEDIFF(今日纽约, dataasof)`（[`asset_managment_config.py:597-598`](https://gitlab.bridgerinvestment.com/jli/prefectflow/-/blob/32a750a39c7eda989de991c47467979043e3d209/flow/bps/bps_config/asset_managment_config.py#L597-598)）。
...
>   - **映射数据在数据库字典表、解码逻辑在代码**（非硬编码）：字典表 `newrez.portnewrezdatadic`（长表 `field_name | code | description`，仅存于 **Redshift**，dev MySQL 无此表）；解码 JOIN 在 [`basic_data_pool_config.py:835-840`](https://gitlab.bridgerinvestment.com/jli/prefectflow/-/blob/32a750a39c7eda989de991c47467979043e3d209/flow/basic_data/basic_data_config/basic_data_pool_config.py#L835-840)（`:835 LMDeal→deal` `:836 LMProgram→program` `:837 LMStatus→lmc_status` `:838 LMDecision→final_disposition` `:839 BorrowerIntention` `:840 DenialReason`；BK 在 `:367 BKStatus`）。`concat(code,'.0')` 用于对齐 `lmdeal` 的小数串存法（如 `'7.0'`）。
...
| UI 列 | BPS 字段 | Newrez 原始字段 | 来源表 | Newrez字段 → BPS字段（ETL 代码核实 [basic_data_pool_config.py:349-363](https://gitlab.bridgerinvestment.com/jli/prefectflow/-/blob/32a750a39c7eda989de991c47467979043e3d209/flow/basic_data/basic_data_config/basic_data_pool_config.py#L349-363)） |
|---|---|---|---|---|
...
- **（已核实，2026-06-02 ETL 代码 + DB 实测）** 经 [`basic_data_pool_config.py:349-363`](https://gitlab.bridgerinvestment.com/jli/prefectflow/-/blob/32a750a39c7eda989de991c47467979043e3d209/flow/basic_data/basic_data_config/basic_data_pool_config.py#L349-363)（BK 提取层）逐字段核实并更正：
...
| Q4 | `currentmilestone` 与 `fcstage` 含义不同：`fcstage` 是 Newrez 内部当前工作步骤，`currentmilestone` 是 BPS 里程碑标签 | `summary_current_step` | ✅ **已解决（2026-06-10 代码+prod 实测）**：`summary_current_step` **= `fcstage` 直传**（`basic_data_pool_config.py:282`），**不取 `currentmilestone`**（全仓 0 引用、ETL 不读）。doc 19 实测与代码一致；详见 Q13 |
...
| Q13 | **【✅ 已解决·2026-06-10 代码核实】** `summary_current_step` 取值规则定论 | `summary_current_step`（并已修正 Q4 / §3.7 / doc 14 / doc 15 / doc 16） | ✅ **结论**：ETL 源 [`basic_data_pool_config.py:282`](https://gitlab.bridgerinvestment.com/jli/prefectflow/-/blob/32a750a39c7eda989de991c47467979043e3d209/flow/basic_data/basic_data_config/basic_data_pool_config.py#L282) = `fc.fcstage as summary_current_step` — **直传 `fcstage`，不含 `currentmilestone`**（`fcstage` 上游又透传自 `portnewrezfc.fcstage`，pool:1560）。`currentmilestone` 在整个 PrefectFlow **grep 0 命中**（ETL 从不读取）。prod `bpms.sync_loan_foreclosure.summary_current_step` 实测值均为 `fcstage` 自由文本（"Pre-Sale Review 1 (SCRA and PACER Check)" / "NOS Sent for Recording" / "Order Of Reference Sent"…），**非** `currentmilestone` 的 7 值枚举。→ 旧「currentmilestone 优先」（原 Q4 / doc 16）**写反**，已统一更正为 `= fcstage`；`currentmilestone` 原始列虽 62.7% 填充但管道未使用，doc 19 实测与代码一致 |
...
| Current Step | Judgment Entered | `fcstage`（`currentmilestone` ETL 未使用） |
