...
**流动顺序 / Flow:** ①portfunding → ②sync_loan_foreclosure → ③biz_data_view_loan_details_foreclosure
**血缘（逐跳：序号 列 — 规则 [代码]）/ Lineage (per hop)**
...
**规则（完整）/ Rule (full):** **Newrez**：原始 `daysinfc` 直传（pool:1546）。**Carrington / Capecodfive**：无原始 `daysinfc` → 计算 `CASE WHEN <active> THEN datediff(day, referral_start_date, <快照日>)+1 ELSE NULL`（Carrington 用 snap_shot_date，pool:1587；Capecodfive 用 dataasof，pool:1628）。下游视图显示延迟校正值，详见逐跳规则。
