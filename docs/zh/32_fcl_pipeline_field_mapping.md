...
| 2026-06-15 | AI Agent (Claude Opus 4.8) | v16 | **Phase 5：补全最后 3 张逐表 sheet（31→34 sheet，至此全 24 表均有逐表 sheet）**。新增 ⑦mid·daily_loan_common(78列)、⑧mid·daily_common_clean(103列)（逾期支线 L2/L3 日表）+ ⑭dim·portfunding(57列，融资池维度)。字段含义本已在 fcl_field_meanings.json（78/103/57 全有 m/src），仅缺举例数据→子agent 只读拉取 25 笔样本最新快照(⑦ asofdate 2026-06-13 / ⑧ fctrdt 2026-07-01 / ⑭ current，25/25 命中)写 fcl_layer_examples_phase5.json(字段名与 FM 100% 对齐)。新增 SRC_LIKE 集令这 3 表走 **src-like 布局**（字段+业务含义+25 举例，无 FCL 公式列）+ 专用 note：⑦⑧ 标注【逾期支线·不流向 FCL group·仅上下文对照】（⑫ 独立重算）、⑭ 标注【FCL/sync 运行时 JOIN 取 funding_id/bid_id 的维度表】。tab 升序插入(⑥|⑦⑧|⑨…⑬|⑭|⑮)、① 索引 3 行由「Phase 待补」变「➤ 打开」、导读去掉「仍无 sheet」警告。真 Excel 741 公式 0 错误、幂等(3febdeae)、34 sheet | fcl_layer_examples_phase5.json；fcl_field_meanings.json |
| 2026-06-15 | AI Agent (Claude Opus 4.8) | v15 | **⑫ 表别名澄清 + 上游血缘 Code-First 订正 + F 页 LT09 澄清**（用户反馈：⑫ 计算逻辑里 g表/l表 是什么表？LT09 看不明白）。① ⑫ fcl_related 的 12 条 rule 全部写出全名（g=newrez.portnewrezgeneral、l=newrez.portnewrezliq、pmt=newrez.portnewrezpmt、bk=newrez.portnewrezbk、pr=newrez.portnewrezprop、c=carrington.portcarrington、r=本表）+ 别名图例 + Carrington 分支源。② Code-First 读 CREATE_FCL_RELATE_ATTR(pool:1696-1770) 订正 ⑫ 上游＝5 张 Newrez 原始表 JOIN UNION carrington.portcarrington（旧误标 basic_data_loan_fcl+daily_loan_common_clean）；delq_status 由 g.delinquency_status_mba+days360(pmt.nextduedate) 现算，逾期支线 ⑦⑧ 不流向 ⑫/⑬ group（独立重算）；连带订正 ⑬/⑦/⑧/⑤/③/⑥/②c 上下游 → 重生成 doc 19 + rebuild KG + re-inject HTML FTMETA/KG，grep 残留 0。③ F 页 LT09 改名「第三方拍下售出日(按 fcresults；实测恒 null)」+ lt_note 详解（字段 timeline_third_party_sold_date_date 恒置 null/pool:270，5743 行 0 非空，条件逻辑未实装）；F 页 col6 改为同时显示「例 loan ｜ 说明注」。真 Excel 741 公式 0 错误、幂等 | pool:1696-1770/270；asset:769；fcl_table_meta/fcl_logic_coverage |
| 2026-06-15 | AI Agent (Claude Opus 4.8) | v14 | **G 字段级逻辑清单「规则摘要」列改为简明逻辑文字**（用户反馈：原代码追踪术语有些没看明白）。新增 `g_summary()` 归一器把 20 种简短术语（sync/upsert/pass-through/直传/GEN_FORECLOSURE…/target_* commented out/auto-increment PK/UNION ALL unpivot/X→Y/view-computed）改写成 13 种白话：拷贝类统一为「本列 := 上游同名列（原样拷贝，值不变）」(+「实测=∅NULL」当空)、改名「本列(Y) := 上游 X（仅改列名）」、target_*「源计算被注释停用⇒恒=∅NULL」、系统列/自增主键/分支/unpivot/视图实时算 各自白话；":=" 读作"取自"。G 页顶部加「读法」图例行(浅黄)解释术语+历史依赖列。**仅改 G 页渲染**（B 页/doc 26/HTML 的逐跳详规则不动，保留代码追踪）。真 Excel 741 公式 0 错误、幂等 | fcl_lineage_source.json（只读引用）；B 页/HTML 留详版 |
| 2026-06-14 | AI Agent (Claude Opus 4.8) | v13 | **⑬ `*_in_lm_days`/`*_on_hold_days` 的公式列(E/F) 从通用样板注 → 「数据代入·分步运算」工作示例**（用户反馈：公式列没拿实际数据运算）。单格活公式选不出「该阶段窗口内 OPEN 未关闭的那一条」LM周期/Hold段(一笔多条，⑯/⑮ 只渲染代表行)，故按 pool 实测归因(in_lm pool:2235-2266 / on_hold pool:2204-2231 / 窗口端 logic_*_end pool:2028-2105 / curr_date pool:1779-1781)把 2 笔 demo 的真实数据①窗口②open周期段③重叠起④重叠止⑤datediff+1 **分步算出**，结果=DB实测=举例列同值（7727003984 demand/first_legal in_lm=398·on_hold=107；7727000806 referral_on_hold=319；curr_date=举例快照日 2026-06-13）；空值注明原因(未进入阶段/窗口内无 open 段/阶段未实际开始)。新增 🟨浅黄 FILL_WEX 样式区别于灰注；真源 `outputs/fcl_stage_worked_examples.json`(DB 只读算+核验 32/32 命中,生成器 `build_fcl_stage_worked_examples.txt`)。活公式 729+数据代入示例 32+注 363；真 Excel 741 公式 0 错误、内容级幂等。NOTE_GEN 审计确认其余 159 字段确不可单格代入 | pool:2204-2266；C列工作示例同源 |
...
- `outputs/fcl_stage_worked_examples.json` — **⑬ in_lm/on_hold 公式列「数据代入·分步运算」工作示例**（2 demo loan × 8 阶段 × {in_lm,on_hold}，DB 只读按 pool 归因算+核验=DB实测；生成器 `outputs/build_fcl_stage_worked_examples.txt`，可重跑）
- `outputs/fcl_layer_examples_phase5.json` — **⑦⑧⑭ 逐表 sheet 举例列真源**（⑦ daily_loan_common / ⑧ daily_common_clean / ⑭ portfunding 的 25 笔样本最新快照值；DB 只读实测，字段名与 fcl_field_meanings.json 对齐）
