...
| 2026-06-08 | AI Agent (Claude Opus 4.8) | v5 | 分类 C 新增两条 **FCL 阶段（6 阶段 / 8 桶 / 15 子阶段）** 与 **FCL episode（一段止赎）** 综合词条；与 doc 21 §0.3 术语说明同源 | doc 21 §0.3/§3 · doc 13 · doc 17/18 |
...
**FCL 阶段（FCL Stages，6 阶段）**

一次止赎案推进所经过的法律里程碑。标准 **6 阶段**顺序：
`DEMAND`(催告) → `REFERRAL`(移交律师) → `FIRST_LEGAL`(首次法律行动) → `SERVICE`(文书送达) → `JUDGEMENT`(法院判决) → `SALE`(拍卖)。
阶段表 `fcl_stage_info` 据此把每笔贷款定位到**当前阶段**并算各阶段耗时（扣除 LM/Hold 重叠的非可控天数）。

- 物理阶段表另含 `NOI`（Demand 与 Referral 之间）与 `PUBLICATION`（Service 与 Judgement 之间）两个桶，对 Newrez 通常为空，故常只列 6 个主阶段——**共 8 桶**。
- BPS 合规 target/actual/var 视图再把阶段细分为 **15 个子阶段**（见 doc 13 §3）。

**相关**：Referral；NOI / Demand Letter；Service；Judgment；Sale；FCL episode；doc 21 §3

---

**FCL episode（止赎事件 / 一段止赎）**

一笔贷款"一次完整的止赎经历"：从进入止赎到退出止赎（被复权 Reinstated / 减损治愈 / 还清 Paid，或走到拍卖 → REO 完结）。

- 一笔贷款被治愈后**可再次进入**止赎，即**新的一个 episode**；多轮尝试用 `(loanid, deal_start)` 作 episode 键。
- 典型 **loan : FCL episode = 1:1**——同一时点至多一段活跃止赎；跨生命周期则可有多段（先后发生）。
- 与 BK 的关系是 **N:N**：一次破产**自动中止**冻结的是**整段止赎**（跨所有阶段，不属某一个阶段），且一个 episode 内借款人可**多次**申请 BK。

**相关**：FCL；BK / Bankruptcy；MFR；FCL 阶段；doc 21 §0.3；doc 17 §1/§5；doc 18 §5
