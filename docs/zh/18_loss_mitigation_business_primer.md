...
> **Deal 值以 Newrez `lmdeal` 解码为准**（Redshift 字典表 `newrez.portnewrezdatadic` field_name='LMDeal'；代码 `basic_data_pool_config.py:835`）。字典定义 13 码，**数据实测出现 8 个**（下表）。注意：deal 是 `Payment Plan`（不是 `Repayment Plan`——后者是其 `program`）。
