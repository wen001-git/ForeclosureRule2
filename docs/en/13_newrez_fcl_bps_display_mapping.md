...
| 2026-06-04 | AI Agent (Claude Opus 4.8) | v35 | **Corrected §3.7 `summary_foreclosure_status` mapping rule** (code + DB verified): the old `activefcflag=1 → use fcstage; =0 → fcresults or fcremovaldesc` was **wrong**. Actual logic ([basic_data_pool_config.py:273](https://gitlab.bridgerinvestment.com/jli/prefectflow/-/blob/32a750a39c7eda989de991c47467979043e3d209/flow/basic_data/basic_data_config/basic_data_pool_config.py#L273) GEN_FCL_DETAIL): `activefcflag=1` → fixed text `'Active Foreclosure'`; `activefcflag=0` and `fcremovaldesc` non-empty → `'Closed Foreclosure:'+fcremovaldesc`; else NULL. `fcstage` actually populates `summary_current_step`; `fcresults` is unused. Also updated: business meaning, §3 header note, two appendix source notes (flagged as stale dev sample), and expanded Type/Current Step/Completed Foreclosure arrow shorthand into full "if…then…". DB: Active 43/Closed 50/NULL 1 all match; 4 stale rows noted. Synced with doc 16/14 | [basic_data_pool_config.py:273](https://gitlab.bridgerinvestment.com/jli/prefectflow/-/blob/32a750a39c7eda989de991c47467979043e3d209/flow/basic_data/basic_data_config/basic_data_pool_config.py#L273) · doc 14 v33 · doc 16 v3 |
...
| 2026-06-03 | AI Agent (Claude Opus 4.8) | v33 | §5 numeric-decode mechanism traced (code + DB verified): lmdeal/lmprogram/lmstatus/lmdecision/borrowerintention/denialreason decode source = **Redshift dict table `newrez.portnewrezdatadic`** (not in dev MySQL); decode JOINs at [`basic_data_pool_config.py:835-840`](https://gitlab.bridgerinvestment.com/jli/prefectflow/-/blob/32a750a39c7eda989de991c47467979043e3d209/flow/basic_data/basic_data_config/basic_data_pool_config.py#L835-840) (BK at :367), not hardcoded; LMDeal dict = 13 codes, 8 observed; noted that a cross-table join `lmdeal=1→Evaluation` is a snapshot-timing artifact | [basic_data_pool_config.py:835-840](https://gitlab.bridgerinvestment.com/jli/prefectflow/-/blob/32a750a39c7eda989de991c47467979043e3d209/flow/basic_data/basic_data_config/basic_data_pool_config.py#L835-840); Redshift portnewrezdatadic; data dictionary |
...
| 2026-06-02 | AI Agent (Claude Opus 4.8) | v30b | [ETL code + DB verified] Section 6 Bankruptcy mapping corrected: `status_date`←`bkfileddate` (not bkrcurrentstatusdate); `lien_status`/`mfr_status`/`claim_status`/`mfr_filed_date` hardcoded NULL in extract; `bankruptcy_status`=COALESCE(datadic decode, bkstatus) | [basic_data_pool_config.py:349-363](https://gitlab.bridgerinvestment.com/jli/prefectflow/-/blob/32a750a39c7eda989de991c47467979043e3d209/flow/basic_data/basic_data_config/basic_data_pool_config.py#L349-363) |
...
- ~~`lien_status` and `claim_status` in the Bankruptcy panel: source Newrez fields require further confirmation~~ **(Resolved 2026-06-02)**: `lien_status` / `claim_status` / `mfr_status` / `mfr_filed_date` are **hardcoded `NULL`** in the ETL extract ([basic_data_pool_config.py:349-363](https://gitlab.bridgerinvestment.com/jli/prefectflow/-/blob/32a750a39c7eda989de991c47467979043e3d209/flow/basic_data/basic_data_config/basic_data_pool_config.py#L349-363)) — not mapped from any Newrez field. See Section 6.
...
| BPS UI Label | Business Meaning | BPS Display Field | Newrez Field | Newrez Field → BPS Field |
|---|---|---|---|---|
| Bid Approval > Approval Status | Bid approval status (BPS-internal column; the FCL ETL never writes it) | `bid_approval_status` | *(no upstream)* | **No Newrez source** — column exists in the `basic_data_loan_foreclosure` DDL ([`basic_data_pool_config.py:193`](https://gitlab.bridgerinvestment.com/jli/prefectflow/-/blob/32a750a39c7eda989de991c47467979043e3d209/flow/basic_data/basic_data_config/basic_data_pool_config.py#L193)) but is **NOT in the INSERT column list**; intended to be set by a BPS-side bid-approval workflow. **DB-verified 2026-06-07: 0% filled** (0/6152 RS · 0/89 BPS) |
...
| Bid Approval > Bid Amount | Bid amount (the ONLY bid_approval_* column the ETL populates) | `bid_approval_bid_amount` | `fcbidamount` | ⚠️ **Corrected**: source is `fcbidamount` ([`basic_data_pool_config.py:272`](https://gitlab.bridgerinvestment.com/jli/prefectflow/-/blob/32a750a39c7eda989de991c47467979043e3d209/flow/basic_data/basic_data_config/basic_data_pool_config.py#L272)), **NOT `fcapprbidprice`**; same value as `summary_foreclosure_bid_amount`. DB-verified 16 rows filled (16/6152 · 16/89) |
...
| Summary > Current Step | Current FCL processing step (Newrez stage description `fcstage`) | `summary_current_step` | `fcstage` | **= `fcstage` direct passthrough** (code [`basic_data_pool_config.py:282`](https://gitlab.bridgerinvestment.com/jli/prefectflow/-/blob/32a750a39c7eda989de991c47467979043e3d209/flow/basic_data/basic_data_config/basic_data_pool_config.py#L282) `fc.fcstage as summary_current_step`). ⚠️ `currentmilestone` exists in `portnewrezfc` (62.7% filled) but is **referenced nowhere in PrefectFlow — the ETL never reads it**; the earlier "currentmilestone-first" conclusion was disproven by code + prod (2026-06-10) and corrected (see §8 Q13) |
...
> - `summary_days_in_fcl` ← `daysinfc`: ETL recomputes from the **referral date `fcreferraldate`** as `datediff(referral_start_date, snapshot)+1` (only when Active; [`basic_data_pool_config.py:1628`](https://gitlab.bridgerinvestment.com/jli/prefectflow/-/blob/32a750a39c7eda989de991c47467979043e3d209/flow/basic_data/basic_data_config/basic_data_pool_config.py#L1628)) — **investor / full-timeline** basis.
...
> - Both get the BPS real-time adjustment `+ DATEDIFF(today_NY, dataasof)` ([`asset_managment_config.py:597-598`](https://gitlab.bridgerinvestment.com/jli/prefectflow/-/blob/32a750a39c7eda989de991c47467979043e3d209/flow/bps/bps_config/asset_managment_config.py#L597-598)).
...
>   - **Mapping data lives in a DB dictionary table; decode logic lives in code** (not hardcoded): dict table `newrez.portnewrezdatadic` (long format `field_name | code | description`, **Redshift-only** — not in dev MySQL); decode JOINs at [`basic_data_pool_config.py:835-840`](https://gitlab.bridgerinvestment.com/jli/prefectflow/-/blob/32a750a39c7eda989de991c47467979043e3d209/flow/basic_data/basic_data_config/basic_data_pool_config.py#L835-840) (`:835 LMDeal→deal`, `:836 LMProgram→program`, `:837 LMStatus→lmc_status`, `:838 LMDecision→final_disposition`, `:839 BorrowerIntention`, `:840 DenialReason`; BK at `:367 BKStatus`). `concat(code,'.0')` aligns with `lmdeal`'s decimal-string storage (e.g. `'7.0'`).
...
| UI Column | BPS Field | Newrez Raw Field | Source Table | Newrez Field → BPS Field (ETL code-verified [basic_data_pool_config.py:349-363](https://gitlab.bridgerinvestment.com/jli/prefectflow/-/blob/32a750a39c7eda989de991c47467979043e3d209/flow/basic_data/basic_data_config/basic_data_pool_config.py#L349-363)) |
|---|---|---|---|---|
...
- **(Resolved 2026-06-02, ETL code + DB-verified)** Verified field-by-field against [`basic_data_pool_config.py:349-363`](https://gitlab.bridgerinvestment.com/jli/prefectflow/-/blob/32a750a39c7eda989de991c47467979043e3d209/flow/basic_data/basic_data_config/basic_data_pool_config.py#L349-363) (BK extract layer):
...
| Q4 | `currentmilestone` and `fcstage` track different things: `fcstage` is Newrez's current workflow step; `currentmilestone` is the BPS milestone label | `summary_current_step` | ✅ **Resolved (2026-06-10, code + prod)**: `summary_current_step` **= `fcstage` direct passthrough** (`basic_data_pool_config.py:282`); `currentmilestone` is **not used** (0 refs in repo, ETL never reads it). doc 19 measurement agrees with the code; see Q13 |
...
| Q13 | **[✅ Resolved · 2026-06-10 code-verified]** `summary_current_step` value rule settled | `summary_current_step` (also corrects Q4 / §3.7 / doc 14 / doc 15 / doc 16) | ✅ **Conclusion**: ETL source [`basic_data_pool_config.py:282`](https://gitlab.bridgerinvestment.com/jli/prefectflow/-/blob/32a750a39c7eda989de991c47467979043e3d209/flow/basic_data/basic_data_config/basic_data_pool_config.py#L282) = `fc.fcstage as summary_current_step` — **direct passthrough of `fcstage`, no `currentmilestone`** (`fcstage` itself passes through from `portnewrezfc.fcstage`, pool:1560). `currentmilestone` has **0 hits in a full PrefectFlow grep** (ETL never reads it). prod `bpms.sync_loan_foreclosure.summary_current_step` values are all `fcstage` free-text ("Pre-Sale Review 1 (SCRA and PACER Check)" / "NOS Sent for Recording" / "Order Of Reference Sent"…), **not** the 7-value `currentmilestone` enum. → the earlier "currentmilestone-first" claim (old Q4 / doc 16) was **reversed**; unified correction to `= fcstage`; the raw `currentmilestone` column, though 62.7% filled, is unused by the pipeline. doc 19 measurement agrees with the code |
...
| Current Step | Judgment Entered | `fcstage` (`currentmilestone` not used by ETL) |
