...
> **Verification SQL**: `fcbidamount`/`fcapprbidprice` fill rates come from Appendix B — SQL-1; sample loan field values can be queried via SQL-4

| BPS UI Label | Business Meaning | BPS Display Field | Newrez Field | Newrez Field → BPS Field |
|---|---|---|---|---|
| Bid Approval > Approval Status | Bid price approval status (BPS internal workflow field; not synced from Newrez) | `bid_approval_status` | *(BPS internal workflow)* | **Not from Newrez** — written by BPS internal bid approval process |
| Bid Approval > Sale Date | Sale date associated with the bid approval (same source as `timeline_sale_date_projected_date`) | `bid_approval_sale_date` | `fcscheduledsaledate` | Direct |
| Bid Approval > Bid Amount | BPS-approved bid price (8.9% fill rate in active FCL loans) | `bid_approval_bid_amount` | `fcapprbidprice` | Direct |
| Bid Approval > Loan Resolution Holds | Current hold reasons blocking loan resolution (note: field name has typo "holods" — this is the original field name) ⚠️ | `bid_approval_loan_resolution_holods` | `fchold1description`, `fchold2description`, `fchold3description` | Concatenate all non-null hold descriptions, separated by `;` |
